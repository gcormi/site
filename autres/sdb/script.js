...
const gameState = {
    ...
    // gameState.currentLyricsHandler a été supprimé
};

// --- La variable songLyrics et la fonction setupLyrics ont été supprimées ---
...
function showPage(pageNumber) {
    ...
    if (targetPage) {
        ...
        // Arrêter la musique des autres pages et gérer la musique de la page actuelle
        const allAudios = document.querySelectorAll('audio');
        allAudios.forEach(aud => {
            // Si l'audio n'est pas celui de la page actuelle ET qu'il est en cours de lecture
            if (aud.id !== `audio-page-${pageNumber}` && !aud.paused) {
                aud.pause();
                aud.currentTime = 0;
            }
        });
        
        const currentPageAudio = document.getElementById(`audio-page-${pageNumber}`);
        // Démarrer la musique pour les pages d'épilogue si elle est en pause
        if (currentPageAudio && currentPageAudio.paused && (pageNumber === 11 || pageNumber === 12)) {
             currentPageAudio.play().catch(error => console.warn(`Lecture auto pour page-${pageNumber} bloquée:`, error));
        }
        
        // La logique de nettoyage du gestionnaire de paroles a été supprimée car plus de synchro

        if (pageNumber === 2) initPendu();
        if (pageNumber === 7) initMotsMeles();
        // L'appel à setupLyrics() a été supprimé
        ...
        const pageContentElements = targetPage.querySelectorAll(
            // Mise à jour du sélecteur pour exclure le nouveau conteneur de paroles statiques
            '.page-content > p, .page-content > ul, .page-content > form, .page-content > div:not(#motsmeles-grille-container):not(.accessibility-options):not(#static-lyrics-container), .page-content > .actions-defi, .page-content > .actions-defi > button, .page-content > input[type="text"], .page-content > select, .page-content .motsmeles-instructions, .blanks-defi6, .accessibility-options, .epilogue-content > h2, .epilogue-content > p, .epilogue-content > ul'
        );
        ...
        setTimeout(() => {
            ...
            // Animer le conteneur des paroles statiques s'il existe
...
function startGame() {
    ...
    // La logique de nettoyage du gestionnaire de paroles a été supprimée
...
function endMotsMelesGame(success) {
    ...
    if(success||actualFound>=motsMelesParams.targetWordsToFind){
        feedback.textContent=`Bravo ! Vous avez trouvé ${actualFound} mot(s) (objectif: ${motsMelesParams.targetWordsToFind}).`;
...
function skipChallenge(challengeIndex) {
    ...
    else if (challengeIndex === 7) message += "L'objectif était de trouver 4 mots.";
